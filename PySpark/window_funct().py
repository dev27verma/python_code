...
from pyspark.sql.functions import row_number, rank, percent_rank, cume_dist, lag, ntile, lead, dense_rank
...
df.withColumn("row",row_number().over(windowSpec)).withColumn("avg", avg(col("salary")).over(windowSpecAgg)) \
  .withColumn("sum", sum(col("salary")).over(windowSpecAgg)).withColumn("min", min(col("salary")).over(windowSpecAgg)) \
  .withColumn("max", max(col("salary")).over(windowSpecAgg)).where(col("row")==1).select("department","avg","sum","min","max").show()