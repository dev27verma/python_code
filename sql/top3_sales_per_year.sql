...
-- write SQL in Bigquery to find top 3 products in terms of total sales amount for each year . Output col shud have prod_nm, year and total_sales

WITH yearly_sales AS (
  SELECT
    p.prod_nm,
    EXTRACT(YEAR FROM PARSE_DATE('%d-%m-%Y', s.order_dt)) AS year,
    SUM(s.sales_amt) AS total_sales
  FROM `project.dataset.sales_table` s
  JOIN `project.dataset.product_table` p
